...
module AVLTree =
    ...
    let rec private treeHeight tree =
        ...
        |E -> 0
        |T(left, _, right) ->
            (Math.Max ((treeHeight left), (treeHeight right))) + 1
            
    let private balanceFactor tree =
        match tree with
        |T(left, _, right) ->
            (treeHeight right) - (treeHeight left)
        |_ -> 0
    ...
    let balanceTree tree =
        ...
        |T(left, item, right) ->
            if balanceFactor tree = 2 then
                rotateLeft (T(left, item,
                            if balanceFactor right < 0 then
                                rotateRight right
                            else
                                right))
            else if balanceFactor tree = -2 then
                rotateRight (T((if balanceFactor left > 0 then
                                    rotateLeft left
                                else
                                    left), item, right))
            else
    ...
    let rec concat tree1 tree2 =
        ...
        |_ ->
            ...
            |E -> tree2
            |T(left, item, right) ->
                insert item tree2 |> concat left |> concat right
    ...
    let rec map f tree =
        ...
        |E -> E
        |T(left, item, right) ->
            T((map f left), f item, (map f right))
    ...
    printTree tree2
    printfn ""
    let tree3 = insert 500 tree |> insert 400 |> insert 600 |> insert 300 |> insert 450 |> insert 700 |> insert 650 |> insert 800 |> delete 400 |> delete 450
    printTree tree3
    printfn ""
    let tree4 = insert 500 E |> insert 400 |> insert 600 |> insert 550 |> insert 700 |> insert 525 |> insert 800
    printTree tree4
    printfn ""
